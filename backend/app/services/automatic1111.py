...
class Automatic1111(IService):
    ...
    async def asyncPost(self, prompt, input_image_64, mask_image_64, cfg_scale, denoising_strength, height, width, steps):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(self.host + "/sdapi/v1/img2img",
                ...
                self.resultImage64 = text
                print("INFO: Automatic async post succeed ")
    ...
    def checkImageGenerationStatus(self):
        if not self.task.done():
            responce_status = requests.get("http://212.33.245.26:63000/sdapi/v1/progress")
    ...
    def stopImageGeneration(self):
        response_stop = requests.post("http://212.33.245.26:63000/sdapi/v1/interrupt")
    ...
    def getGeneratedImage(self):
        return "data:image/png;base64," + self.resultImage64
    ...
    def checkImageInterrogationStatus(self):
        status = self.taskPrompt.done()
        if status:
            print("INFO: Interrogtion on Automatic complete")
            return True
        else:
            print("INFO: Interrogtion on Automatic in process...")
            return False
